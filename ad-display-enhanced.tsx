...
interface AdDisplayEnhancedProps {
  placement: "header" | "sidebar-top" | "sidebar-middle" | "in-article" | "footer";
  currentPage?: string;
  className?: string;
}

export function AdDisplayEnhanced({ placement, currentPage = "home", className = "" }: AdDisplayEnhancedProps) {
    ...
    const fetchAds = async () => {
      try {
        ...
        const filteredAds = data.filter((ad: Ad) => {
          if (!ad.targetPages || ad.targetPages.length === 0) return true; // عرض في جميع الصفحات
  ...
  return (
    <div className={`ad-display-enhanced ${className}`}>
      <Card
        className={`${getPlacementDimensions(placement)} overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer group relative border-0`}
        ...
        {currentAd.filePath && (
          <div className="relative w-full h-full overflow-hidden bg-gray-100 dark:bg-gray-800">
            <img
              ...
              onError={(e) => {
                console.error("[AdDisplay] Image load error:", currentAd.filePath);
